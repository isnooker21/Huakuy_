...
class Dynamic7DSmartCloser:
    ...
    def __init__(self, purpose_tracker=None, market_analyzer=None, price_action_analyzer=None):
        ...
        # 🧠 INTELLIGENT Parameters - OPTIMIZED (ลด complexity)
        self.base_safety_buffer = 0.0  # ไม่มีเกณฑ์คงที่ - ให้ระบบตัดสินใจเอง
        self.base_max_group_size = 10  # ลดสูงสุดให้ระบบเร็วขึ้น (ลดจาก 50)
    ...
    def _smart_profit_optimized_selection(self, positions: List[Any], size: int) -> List[Any]:
        ...
        try:
            ...
            # เรียงไม้ขาดทุนตาม loss (ขาดทุนมากสุดก่อน - ฉลาดในการเลือกไม้แย่)
            ...
            if profitable_sorted and losing_sorted:
                ...
                for profit_count in range(1, max_profit_count + 1):
                    ...
                    # เลือกไม้ขาดทุนมากที่สุด (ฉลาดในการเลือกไม้แย่ - ไม้ที่อยู่ไกลราคาปัจจุบัน)
            ...
            if not profitable_sorted and losing_sorted:
                logger.warning("⚠️ ZERO LOSS POLICY: ไม่มีไม้กำไรเลย - ไม่ปิดไม้เลย (ไม่ปิดขาดทุน)")
                # ไม่เลือกไม้ขาดทุนเลย ตาม ZERO LOSS POLICY
