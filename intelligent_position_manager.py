...
class IntelligentPositionManager:
    ...
    def __init__(self, mt5_connection, order_manager, symbol: str = "XAUUSD"):
        ...
        # 🎯 เกณฑ์การตัดสินใจแบบ Dynamic - ปรับให้สอดคล้องกัน
        self.margin_thresholds = {
            'critical': 100,    # Margin Level < 100% = วิกฤต (ปรับจาก 150)
            'high_risk': 200,   # < 200% = เสี่ยงสูง (ปรับจาก 300)
            'medium_risk': 400, # < 400% = เสี่ยงปานกลาง (ปรับจาก 500)
            'safe': 800         # > 800% = ปลอดภัย (ปรับจาก 1000)
        }
    ...
    def _make_intelligent_decision(self, position_scores: List[PositionScore], 
        ...
        try:
            ...
            # 💰 INTELLIGENT POSITIVE SUM CLOSING: ใช้ 7-dimensional scoring หาชุดที่ผลรวมบวกเสมอ
