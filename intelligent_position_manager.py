...
class IntelligentPositionManager:
    ...
    def __init__(self, mt5_connection, order_manager, symbol: str = "XAUUSD"):
        ...
        # 🔥 AGGRESSIVE THRESHOLDS: สู้ให้สุด - เสี่ยงได้มากขึ้น!
        self.margin_thresholds = {
            'critical': 30,     # Margin Level < 30% = วิกฤตจริงๆ (ลดจาก 100)
            'high_risk': 80,    # < 80% = เสี่ยงสูง (ลดจาก 200)
            'medium_risk': 150, # < 150% = เสี่ยงปานกลาง (ลดจาก 400)
            'safe': 300         # > 300% = ปลอดภัย (ลดจาก 800)
        }
